 ...
 * Subject : CJ Mall 운영 
 * Name : TC_004
 * Scenario : 비로그인 상태에서 "마이존" 클릭시 로그인 화면 전환 확인
 * Assertion : 1. "로그인" Text 체크
 *             2. 로그인 버튼 value 확인 체크
 *
 */

public class Live_TC004 {
	...
	public void TC004() throws Exception {
		try{
			...
			// 로그인 Text 체크
			assertEquals("로그인", driver.findElement(By.xpath(".//*[@id='cjo_wrap']/div[2]/div/div/div[2]/ul/li[1]/a/span")).getText());
			System.out.println("로그인 Text 체크");
			...
			// 로그인 버튼 value 확인 체크
			if (driver.findElement(By.xpath(".//*[@id='login_area']/form/div/div/div[2]/input")).getAttribute("value").equals("로그인")) {
				System.out.println("[TC_004] success");
				assertTrue(true);
				return;
			} else {
				System.out.println("[TC_004] failure : 로그인 버튼 value 불일치");
				assertTrue(false);
			}
		} catch (Exception e) {
			e.printStackTrace();
			System.out.println("[TC_004] failure");
			assertTrue(false);
		}
	}
