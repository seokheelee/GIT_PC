 ...
 * Subject : CJ Mall 운영
 * Name : TC_001
 * Scenario : "로그인" 클릭시 로그인 화면 전환 확인
 * Assertion : 로그인 버튼 value 확인 체크
 *
 */

public class Live_TC001 {
	...
	public void TC001() throws Exception {
		try{
			...
			// 로그인 버튼 value 확인 체크
			boolean isExist = existElement(driver, By.xpath(".//*[@id='login_area']/form/div/div/div[2]/input"), "로그인");
			if (isExist && driver.findElement(By.xpath(".//*[@id='login_area']/form/div/div/div[2]/input")).getAttribute("value").equals("로그인")) {
				System.out.println("[TC_001] success");
				assertTrue(true);
				return;
			} else {
				System.out.println("[TC_001] failure : 로그인 버튼 value 불일치");
				assertTrue(false);
			}
		} catch (Exception e) {
			e.printStackTrace();
			System.out.println("[TC_001] failure");
			assertTrue(false);
		}
	}
