 ...
 * Subject : CJ Mall 운영 
 * Name : TC_127
 * Scenario : 로그인 > 마이존 > 할인쿠폰 클릭시 해당 탭 이동 확인
 * Assertion : "고객님께서 사용 가능한 할인쿠폰은 ~장입니다." Text 체크
 *
 */

public class Live_TC127 {
	...
	@Test
	public void TC127() throws Exception {
		try {
			...
			// 할인쿠폰 클릭
			driver.findElement(By.xpath(".//*[@id='wrap']/div[1]/div[2]/div[2]/div[2]/table/tbody/tr[1]/th[1]/strong/a")).click();
			System.out.println("할인쿠폰 클릭");
			
			// Text 뜰 때까지 기다림.
			wait = new WebDriverWait(driver, waitTime);
			wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath(".//*[@id='sub2_right']/div/div[2]/div[3]/form/ul/li[5]")));
			System.out.println("Text 뜰 때까지 기다림");
			
			// "고객님께서 사용 가능한 할인쿠폰은 ~장입니다." Text null 여부 체크
			if (!"".equals(driver.findElement(By.xpath(".//*[@id='sub2_right']/div/div[2]/div[3]/form/ul/li[5]")).getText())) {
				System.out.println("[TC_127] success");
				assertTrue(true);
				return;
			} else {
				System.out.println("[TC_127] failure : '고객님께서 사용 가능한 할인쿠폰은 ~장입니다.' Text null...");
				assertTrue(false);
			}
		} catch (Exception e) {
			e.printStackTrace();
			System.out.println("[TC_127] failure");
			assertTrue(false);
		}
	}
