 ...
 * Subject : CJ Mall 운영 
 * Name : TC_130
 * Scenario : 로그인 > 마이존 > 적립금 클릭시 해당 탭 이동 확인
 * Assertion : "사용가능 적립금" Text 체크
 *
 */

public class Live_TC130 {
	...
	public void TC130() throws Exception {
		try {
			...
			// 적립금 클릭
			driver.findElement(By.xpath(".//*[@id='wrap']/div[1]/div[2]/div[2]/div[2]/table/tbody/tr[1]/th[2]/strong/a")).click();
			...
			if ("사용가능 적립금".equals(driver.findElement(By.xpath(".//*[@id='sub2_right']/div/div[3]/table/tbody/tr[1]/td[1]")).getText())) {
				System.out.println("[TC_130] success");
				assertTrue(true);
				return;
			} else {
				System.out.println("[TC_130] failure : '사용가능 적립금' Text 불일치");
				assertTrue(false);
			}
		} catch (Exception e) {
			e.printStackTrace();
			System.out.println("[TC_130] failure");
			assertTrue(false);
		}
	}
