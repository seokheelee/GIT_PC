 ...
 * Subject : CJ Mall 운영 
 * Name : TC_131
 * Scenario : 로그인 > 마이존 > CJ ONE 포인트 클릭시 해당 탭 이동 확인
 * Assertion : "사용가능 포인트" Text 체크
 *
 */

public class Live_TC131 {
	...
	public void TC131() throws Exception {
		try {
			...
			// CJ ONE 포인트 클릭
			driver.findElement(By.xpath(".//*[@id='wrap']/div[1]/div[2]/div[2]/div[2]/table/tbody/tr[2]/th[2]/strong/a")).click();
			System.out.println("CJ ONE 포인트 클릭");
			
			// CJ ONE 포인트 뜰 때까지 기다림.
			wait = new WebDriverWait(driver, waitTime);
			wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath(".//*[@id='sub2_right']/div/div[3]/table/tbody/tr[1]/td[1]")));
			System.out.println("CJ ONE 포인트 뜰 때까지 기다림");
						
			// "사용가능 포인트" Text 체크
			if ("사용가능 포인트".equals(driver.findElement(By.xpath(".//*[@id='sub2_right']/div/div[3]/table/tbody/tr[1]/td[1]")).getText())) {
				System.out.println("[TC_131] success");
				assertTrue(true);
				return;
			} else {
				System.out.println("[TC_131] failure : '사용가능 포인트' Text 불일치");
				assertTrue(false);
			}
		} catch (Exception e) {
			e.printStackTrace();
			System.out.println("[TC_131] failure");
			assertTrue(false);
		}
	}
