 ...
 * Subject : CJ Mall 운영 
 * Name : TC_194
 * Scenario : 로그인 > 마이존 > 쇼핑통장 > 적립금 > 사용가능 적립금 노출
 * Assertion : 사용가능 적립금 : "~원" Text 체크
 *
 */

public class Live_TC194 {
	...
	@Test
	public void TC194() throws Exception {
		try {
			...
			System.out.println("사용가능 적립금 뜰 때까지 기다림");
			
			// 사용가능 적립금 : "~원" Text 체크
			if ("사용가능 적립금".equals(driver.findElement(By.xpath(".//*[@id='sub2_right']/div/div[3]/table/tbody/tr[1]/td[1]")).getText())) {
				System.out.println("[TC_194] success");
				assertTrue(true);
				return;
			} else {
				System.out.println("[TC_194] failure : '사용가능 적립금 : ~원' Text 불일치");
				assertTrue(false);
			}
		} catch (Exception e) {
			e.printStackTrace();
			System.out.println("[TC_194] failure");
			assertTrue(false);
		}
	}
