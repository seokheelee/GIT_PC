 ...
 * Subject : CJ Mall 운영 
 * Name : TC_200
 * Scenario : 로그인 > 마이존 > 쇼핑통장 > CJ ONE 포인트 > 사용가능 포인트 노출
 * Assertion : 사용가능 포인트 : "~P" Text 체크
 *
 */

public class Live_TC200 {
	...
	@Test
	public void TC200() throws Exception {
		try {
			...
			System.out.println("사용가능 포인트 뜰 때까지 기다림");
			
			// 사용가능 포인트 : "~P" Text 체크
			if ("사용가능 포인트".equals(driver.findElement(By.xpath(".//*[@id='sub2_right']/div/div[3]/table/tbody/tr[1]/td[1]")).getText())) {
				System.out.println("[TC_200] success");
				assertTrue(true);
				return;
			} else {
				System.out.println("[TC_200] failure : '사용가능 포인트 : ~P' Text 불일치");
				assertTrue(false);
			}
		} catch (Exception e) {
			e.printStackTrace();
			System.out.println("[TC_200] failure");
			assertTrue(false);
		}
	}
