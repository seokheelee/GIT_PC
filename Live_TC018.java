 ...
 * Subject : CJ Mall 운영 
 * Name : TC_018
 * Scenario : TV쇼핑 상단 카테고리 중 "생방송" 클릭시 생방송 화면 이동 확인
 * Assertion : "TV쇼핑>생방송/편성표보기" Text 체크
 *
 */

public class Live_TC018 {
	...
	public void TC018() throws Exception {
		try{
			...
			// 생방송 클릭
			driver.findElement(By.xpath(".//*[@id='cjm_container']/div[1]/div[2]/div/ul/li[1]/strong/a")).click();
			System.out.println("생방송 클릭");
			
			// TV쇼핑 > 생방송/편성표보기 Text 기다림
			wait = new WebDriverWait(driver, waitTime);
			wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath(".//*[@id='locationEtcDiv']")));
			System.out.println("TV쇼핑 > 생방송/편성표보기 Text 기다림");
			
			// TV쇼핑 > 생방송/편성표보기 Text find
			String n1 = driver.findElement(By.xpath(".//*[@id='locationEtcDiv']")).getText();
			System.out.println("TV쇼핑 > 생방송/편성표보기 Text find");
			
			// TV쇼핑 > 생방송/편성표보기 Text 비교
			if("TV쇼핑 > 생방송/편성표보기".equals(n1)){
				System.out.println("[TC_018] success");
				assertTrue(true);
				return;
			} else {
				System.out.println("[TC_018] failure : 'TV쇼핑 > 생방송/편성표보기' Text 불일치");
				assertTrue(false);
			}
		} catch (Exception e) {
			e.printStackTrace();
			System.out.println("[TC_018] failure");
			assertTrue(false);
		}
	}
