 ...
 * Subject : CJ Mall 운영 
 * Name : TC_113
 * Scenario : 비로그인 > 장바구니 선택 시 로그인 페이지 이동 확인
 * Assertion : 로그인 img alt 확인
 *
 */

public class Live_TC113 {
	...
	public void TC113() throws Exception {
		try{
			...
			// 메인 페이지 뜨고, 장바구니 객체가 뜰 때까지 기다림.
			wait = new WebDriverWait(driver, waitTime);
			wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath(".//*[@id='cjo_wrap']/div[2]/div/div/div[2]/ul/li[4]/a")));
			System.out.println("메인 페이지 뜨고, 장바구니 객체가 뜰 때까지 기다림");
			...
			System.out.println("로그인 img 뜰 때까지 기다림");
			
			// 로그인 img alt 확인
			if (driver.findElement(By.xpath(".//*[@id='login_area']/form/h2/img")).getAttribute("alt").equals("로그인")) {
				System.out.println("[TC_113] success");
				assertTrue(true);
				return;
			} else {
				System.out.println("[TC_113] failure : '로그인' img alt 불일치");
				assertTrue(false);
			}
		} catch (Exception e) {
			e.printStackTrace();
			System.out.println("[TC_113] failure");
			assertTrue(false);
		}
	}
