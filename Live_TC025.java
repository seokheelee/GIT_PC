 ...
 * Subject : CJ Mall 운영 
 * Name : TC_025
 * Scenario : TV쇼핑 상단 카테고리 중 "방송 알리미" 카테고리 선택시 방송 알리미 화면 이동 확인
 * Assertion : "TV쇼핑>방송알리미" Text 체크
 *
 */
...
public class Live_TC025 {
	...
	public void TC025() throws Exception {
		try{
			...
			// 방송 알리미 뜰 때까지 기다림.
			wait = new WebDriverWait(driver, waitTime);
			wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath(".//*[@id='cjm_container']/div[1]/div[1]/div[2]/div[2]/ul/li[4]/a/img")));
			System.out.println("방송 알리미 뜰 때까지 기다림");
			
			// 방송 알리미 클릭
			driver.findElement(By.xpath(".//*[@id='cjm_container']/div[1]/div[1]/div[2]/div[2]/ul/li[4]/a/img")).click();
			...
			// "TV쇼핑>방송알리미" Text find
			String n1 = driver.findElement(By.xpath(".//*[@id='locationEtcDiv']")).getText();
			System.out.println("'TV쇼핑>방송알리미' Text find");
			
			// "TV쇼핑>방송알리미" Text 비교
			if("TV쇼핑 > 방송알리미".equals(n1)){
				System.out.println("[TC_025] success");
				assertTrue(true);
				return;
			} else {
				System.out.println("[TC_025] failure : 'TV쇼핑 > 방송알리미' Text 불일치");
				assertTrue(false);
			}
		} catch (Exception e) {
			e.printStackTrace();
