 ...
 * Subject : CJ Mall 운영 
 * Name : TC_195
 * Scenario : 로그인 > 마이존 > 쇼핑통장 > 적립금 > 당월 소멸예정 적립금 노출
 * Assertion : 당월 소멸예정 적립금 : "~원" Text 체크
 *
 */

public class Live_TC195 {
	...
	@Test
	public void TC195() throws Exception {
		try {
			...
			// 적립금 버튼 뜰 때까지 기다림.
			wait = new WebDriverWait(driver, waitTime);
			wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath(".//*[@id='midLeft']/div/ul/li[2]/ul/li[2]/a")));
			System.out.println("적립금 버튼 뜰 때까지 기다림");
			
			// 적립금 클릭
			driver.findElement(By.xpath(".//*[@id='midLeft']/div/ul/li[2]/ul/li[2]/a")).click();
			System.out.println("적립금 클릭");
			
			// 당월 소멸예정 적립금 뜰 때까지 기다림.
			wait = new WebDriverWait(driver, waitTime);
			wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath(".//*[@id='sub2_right']/div/div[3]/table/tbody/tr[2]/td[1]")));
			System.out.println("당월 소멸예정 적립금 뜰 때까지 기다림");
			
			// 당월 소멸예정 적립금 : "~원" Text 체크
			if ("당월 소멸예정 적립금".equals(driver.findElement(By.xpath(".//*[@id='sub2_right']/div/div[3]/table/tbody/tr[2]/td[1]")).getText())) {
				System.out.println("[TC_195] success");
				assertTrue(true);
				return;
			} else {
				System.out.println("[TC_195] failure : '당월 소멸예정 적립금 : ~원' Text 불일치");
				assertTrue(false);
			}
		} catch (Exception e) {
			e.printStackTrace();
			System.out.println("[TC_195] failure");
			assertTrue(false);
		}
	}
