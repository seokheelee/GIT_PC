 ...
 * Subject : CJ Mall 운영 
 * Name : TC_125
 * Scenario : 로그인 > 마이존 > 예치금  노출
 * Assertion : "예치금 ~원" Text 체크
 *
 */

public class Live_TC125 {
	...
	@Test
	public void TC125() throws Exception {
		try {
			...
			// 예치금 뜰 때까지 기다림.
			wait = new WebDriverWait(driver, waitTime);
			wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath(".//*[@id='wrap']/div[1]/div[2]/div[2]/div[2]/table/tbody/tr[2]/th[3]/strong/a")));
			System.out.println("예치금 뜰 때까지 기다림");
			
			// "예치금 ~원" Text 체크
			if ("예치금".equals(driver.findElement(By.xpath(".//*[@id='wrap']/div[1]/div[2]/div[2]/div[2]/table/tbody/tr[2]/th[3]/strong/a")).getText())) {
				System.out.println("[TC_125] success");
				assertTrue(true);
				return;
			} else {
				System.out.println("[TC_125] failure : '예치금 ~원' Text 불일치");
				assertTrue(false);
			}
		} catch (Exception e) {
			e.printStackTrace();
			System.out.println("[TC_125] failure");
			assertTrue(false);
		}
	}
