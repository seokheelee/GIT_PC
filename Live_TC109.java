 ...
 * Subject : CJ Mall 운영 
 * Name : TC_109
 * Scenario : 아이디 저장 체크시, 이 후 로그인 시  아이디 저장 및 노출 확인
 * Assertion : 로그인창 내 기존 입력 아이디 자동입력 여부 확인
 *
 */

public class Live_TC109 {
	...
	public void TC109() throws Exception {
		try{
			...
			// 아이디 저장 클릭
			driver.findElement(By.xpath(".//*[@id='auto_id']")).click();
						
			driver.findElement(By.xpath(".//*[@id='login_area']/form/div/div/div[2]/input")).click();
			System.out.println("아이디 저장 클릭하여 로그인");
			...
			//// 로그아웃 클릭 후 로그인 기다림.
			wait = new WebDriverWait(driver, waitTime);
			wait.until(ExpectedConditions.textToBePresentInElementLocated(By.xpath(".//*[@id='cjo_wrap']/div[2]/div/div/div[2]/ul/li[1]/a/span"), "로그인"));
			System.out.println("로그아웃 클릭 후 로그인 기다림");
			...
			// inputbox 에 값 뿌리는 시간 기다림.
			Thread.sleep(3000);
			System.out.println("99="+driver.findElement(By.xpath(".//*[@id='mid_1']")).getAttribute("value"));
			
			// 아이디 값 저장 비교 확인
			if(userId.equals(driver.findElement(By.xpath(".//*[@id='mid_1']")).getAttribute("value"))){
				System.out.println("[TC_109] success");
				assertTrue(true);
				return;
			} else {
				System.out.println("[TC_109] failure : 아이디 저장 값 불일치");
				assertTrue(false);
			}
		} catch (Exception e) {
			e.printStackTrace();
			System.out.println("[TC_109] failure");
			assertTrue(false);
		}
	}
