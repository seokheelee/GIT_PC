 ...
 * Subject : CJ Mall 운영 
 * Name : TC_138
 * Scenario : 로그인 > 마이존 > 다운 가능한 적립금 바로가기 이동 확인
 * Assertion : "사용가능 적립금" Text 체크
 *
 */

public class Live_TC138 {
	...
	public void TC138() throws Exception {
		try {
			...
			// 바로가기 버튼 뜰 때까지 기다림.
			wait = new WebDriverWait(driver, waitTime);
			wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath(".//*[@id='sub2_right']/div/div[1]/div/dl/dd[1]/a/span")));
			System.out.println("바로가기 버튼 뜰 때까지 기다림");
			
			// 다운 가능한 적립금 바로가기 버튼 클릭
			driver.findElement(By.xpath(".//*[@id='sub2_right']/div/div[1]/div/dl/dd[1]/a/span")).click();
			System.out.println("다운 가능한 적립금 바로가기 버튼 클릭");
			
			// 사용가능 적립금 Text 뜰 때까지 기다림.
			wait = new WebDriverWait(driver, waitTime);
			wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath(".//*[@id='sub2_right']/div/div[3]/table/tbody/tr[1]/td[1]")));
			System.out.println("사용가능 적립금 Text 뜰 때까지 기다림");
						
			// "사용가능 적립금" Text 체크
			if ("사용가능 적립금".equals(driver.findElement(By.xpath(".//*[@id='sub2_right']/div/div[3]/table/tbody/tr[1]/td[1]")).getText())) {
				System.out.println("[TC_138] success");
				assertTrue(true);
				return;
			} else {
				System.out.println("[TC_138] failure : '사용가능 적립금' Text 불일치");
				assertTrue(false);
			}
		} catch (Exception e) {
			e.printStackTrace();
			System.out.println("[TC_138] failure");
			assertTrue(false);
		}
	}
