 ...
 * Subject : CJ Mall 운영 
 * Name : TC_250
 * Scenario : 회원정보 > 쇼핑알리미 설정 > 방송알리미 탭 > 방송알리미 등록하기 클릭시 페이지 이동
 * Assertion : TV 방공 알리미 img alt "방송 알리미" 체크
 *
 */

public class Live_TC250 {
	...
	@Test
	public void TC250() throws Exception {
		try {
			...
			// 방송알리미 등록하기 버튼 뜰 때까지 기다림.
			wait = new WebDriverWait(driver, waitTime);
			wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath(".//*[@id='sub2_right']/div/div[1]/div[2]/ul/li[1]/a/img")));
			System.out.println("방송알리미 등록하기 버튼 뜰 때까지 기다림");
			
			// 방송 알리미 버튼 클릭
			driver.findElement(By.xpath(".//*[@id='sub2_right']/div/div[1]/div[2]/ul/li[1]/a/img")).click();
			System.out.println("방송 알리미 버튼 클릭");
			
			// TV쇼핑 이미지 확인
			wait = new WebDriverWait(driver, waitTime);
			wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath(".//*[@id='tvshopping_alimi']/h2/div/img")));
			System.out.println("TV쇼핑 이미지 확인");
			
			// 방송알리미 img 확인
			if("방송 알리미".equals(driver.findElement(By.xpath(".//*[@id='tvshopping_alimi']/h2/div/img")).getAttribute("alt"))){
				System.out.println("[TC_250] success");
				assertTrue(true);
				return;
			} else {
				System.out.println("[TC_250] failure : '방송 알리미' img alt 불일치");
				assertTrue(false);
			}
		} catch (Exception e) {
			e.printStackTrace();
			System.out.println("[TC_250] failure");
			assertTrue(false);
		}
	}
