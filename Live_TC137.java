 ...
 * Subject : CJ Mall 운영 
 * Name : TC_137
 * Scenario : 로그인 > 마이존 > 다운 가능한 CJ ONE 포인트 노출
 * Assertion : 다운 가능한 CJ ONE 포인트 : "~P" Text 체크
 *
 */

public class Live_TC137 {
	...
	public void TC137() throws Exception {
		try {
			...
			// Text 뜰 때까지 기다림.
			wait = new WebDriverWait(driver, waitTime);
			wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath(".//*[@id='sub2_right']/div/div[1]/div/dl/dd[2]/strong")));
			System.out.println("Text 뜰 때까지 기다림");
						
			// 다운 가능한 CJ ONE 포인트 : "~P" Text 체크
			if (!"".equals(driver.findElement(By.xpath(".//*[@id='sub2_right']/div/div[1]/div/dl/dd[2]/strong")).getText())) {
				System.out.println("[TC_137] success");
				assertTrue(true);
				return;
			} else {
				System.out.println("[TC_137] failure : '다운 가능한 CJ ONE 포인트 : ~P' Text 불일치");
				assertTrue(false);
			}
		} catch (Exception e) {
			e.printStackTrace();
			System.out.println("[TC_137] failure");
			assertTrue(false);
		}
	}
