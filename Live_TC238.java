 ...
 * Subject : CJ Mall 운영 
 * Name : TC_238
 * Scenario : 로그인 > 마이존 > 회원정보 > 개인정보 변경 > 비밀번호 입력시 개인정보 변경 탭 페이지 이동
 * Assertion : 하단의 버튼 개인정보 변경 img alt "정보변경 확인" 체크
 *
 */

public class Live_TC238 {
	...
	@Test
	public void TC238() throws Exception {
		try {
			...
			// 개인정보 변경 img alt 체크
			if ("정보변경 확인".equals(driver.findElement(By.xpath(".//*[@id='sub2_right']/div/form/p/a[1]/img")).getAttribute("alt"))) {
				System.out.println("[TC_238] success");
				assertTrue(true);
				return;
			} else {
				System.out.println("[TC_238] failure : '정보변경 확인' img alt 불일치");
				assertTrue(false);
			}
		} catch (Exception e) {
			e.printStackTrace();
			System.out.println("[TC_238] failure");
			assertTrue(false);
		}
	}
