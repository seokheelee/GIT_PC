 ...
 * Subject : CJ Mall 운영 
 * Name : TC_136
 * Scenario : 로그인 > 마이존 > 다운 가능한 적립금 노출
 * Assertion : 다운 가능한 적립금 : "~원" Text 체크
 *
 */

public class Live_TC136 {
	...
	public void TC136() throws Exception {
		try {
			...
			// Text 뜰 때까지 기다림.
			wait = new WebDriverWait(driver, waitTime);
			wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath(".//*[@id='sub2_right']/div/div[1]/div/dl/dd[1]/strong")));
			System.out.println("Text 뜰 때까지 기다림");
			
			System.out.println("ss="+driver.findElement(By.xpath(".//*[@id='sub2_right']/div/div[1]/div/dl/dd[1]/strong")).getText());
						
			// 다운 가능한 적립금 : "~원" Text 체크
			if (!"".equals(driver.findElement(By.xpath(".//*[@id='sub2_right']/div/div[1]/div/dl/dd[1]/strong")).getText())) {
				System.out.println("[TC_136] success");
				assertTrue(true);
				return;
			} else {
				System.out.println("[TC_136] failure : '다운 가능한 적립금 : ~원' Text 불일치");
				assertTrue(false);
			}
		} catch (Exception e) {
			e.printStackTrace();
			System.out.println("[TC_136] failure");
			assertTrue(false);
		}
	}
