 ...
 * Subject : CJ Mall 운영 
 * Name : TC_128
 * Scenario : 로그인 > 마이존 > 쇼핑플러스쿠폰 클릭시 해당 탭 이동 확인
 * Assertion : "쇼핑플러스쿠폰은 일반 할인쿠폰과 중복해서  사용하실 수 있는 쿠폰입니다." Text 체크
 *
 */

public class Live_TC128 {
	...
	@Test
	public void TC128() throws Exception {
		try {
			...
			System.out.println("마이존 창이 뜰 때까지 기다림");
						
			// 쇼핑플러스쿠폰 클릭
			driver.findElement(By.xpath(".//*[@id='wrap']/div[1]/div[2]/div[2]/div[2]/table/tbody/tr[2]/th[1]/strong/a")).click();
			System.out.println("쇼핑플러스쿠폰 클릭");
			
			// Text 뜰 때까지 기다림.
			wait = new WebDriverWait(driver, waitTime);
			wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath(".//*[@id='sub2_right']/div/div[2]/div/form/ul/li[5]")));
			System.out.println("Text 뜰 때까지 기다림");
			
			// "쇼핑플러스쿠폰은 일반 할인쿠폰과 중복해서  사용하실 수 있는 쿠폰입니다." Text 체크
			if (!"".equals(driver.findElement(By.xpath(".//*[@id='sub2_right']/div/div[2]/div/form/ul/li[5]")).getText())) {
				System.out.println("[TC_128] success");
				assertTrue(true);
				return;
			} else {
				System.out.println("[TC_128] failure : '쇼핑플러스쿠폰은 일반 할인쿠폰과 중복해서  사용하실 수 있는 쿠폰입니다.' Text null...");
				assertTrue(false);
			}
		} catch (Exception e) {
			e.printStackTrace();
			System.out.println("[TC_128] failure");
			assertTrue(false);
		}
	}
