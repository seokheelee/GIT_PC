 ...
 * Subject : CJ Mall 운영 
 * Name : TC_139
 * Scenario : 로그인 > 마이존 > 다운 가능한 CJ ONE 포인트 바로가기 이동 확인
 * Assertion : "사용가능 포인트" Text 체크
 * Update : 불필요한 코드의 제거 (2016.11.11)
 *
 */

public class Live_TC139 {
	...
	@Test
	public void TC139() throws Exception {
		try {
			...
			// 바로가기 버튼 뜰 때까지 기다림.
			wait = new WebDriverWait(driver, waitTime);
			wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath(".//*[@id='sub2_right']/div/div[1]/div/dl/dd[2]/a/span")));
			System.out.println("바로가기 버튼 뜰 때까지 기다림");
			
			// 다운 가능한 CJ ONE 포인트 바로가기 버튼 클릭
			driver.findElement(By.xpath(".//*[@id='sub2_right']/div/div[1]/div/dl/dd[2]/a/span")).click();
			System.out.println("다운 가능한 CJ ONE 포인트 바로가기 버튼 클릭");
			
			// 사용가능 포인트 Text 뜰 때까지 기다림.
			wait = new WebDriverWait(driver, waitTime);
			wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath(".//*[@id='sub2_right']/div/div[3]/table/tbody/tr[1]/td[1]")));
			System.out.println("사용가능 포인트 Text 뜰 때까지 기다림");
						
			// "사용가능 포인트" Text 체크
			if ("사용가능 포인트".equals(driver.findElement(By.xpath(".//*[@id='sub2_right']/div/div[3]/table/tbody/tr[1]/td[1]")).getText())) {
				System.out.println("[TC_139] success");
				assertTrue(true);
				return;
			} else {
				System.out.println("[TC_139] failure : '사용가능 포인트' Text 체크");
				assertTrue(false);
			}
		} catch (Exception e) {
			e.printStackTrace();
			System.out.println("[TC_139] failure");
			assertTrue(false);
		}
	}
