 ...
 * Subject : CJ Mall 운영 
 * Name : TC_120
 * Scenario : 로그인 > 마이존 > 쇼핑플러스 쿠폰 노출
 * Assertion : "쇼핑플러스쿠폰 ~개" Text 체크
 *
 */

public class Live_TC120 {
	...
	@Test
	public void TC120() throws Exception {
		try {
			...
			// 쇼핑플러스쿠폰 뜰 때까지 기다림.
			wait = new WebDriverWait(driver, waitTime);
			wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath(".//*[@id='wrap']/div[1]/div[2]/div[2]/div[2]/table/tbody/tr[2]/th[1]/strong/a")));
			System.out.println("쇼핑플러스쿠폰 뜰 때까지 기다림");
			
			// "쇼핑플러스쿠폰 ~개" Text 체크
			if ("쇼핑플러스쿠폰".equals(driver.findElement(By.xpath(".//*[@id='wrap']/div[1]/div[2]/div[2]/div[2]/table/tbody/tr[2]/th[1]/strong/a")).getText())) {
				System.out.println("[TC_120] success");
				assertTrue(true);
				return;
			} else {
				System.out.println("[TC_120] failure : '쇼핑플러스쿠폰' Text 불일치");
				assertTrue(false);
			}
		} catch (Exception e) {
			e.printStackTrace();
			System.out.println("[TC_120] failure");
			assertTrue(false);
		}
	}
