 ...
 * Subject : CJ Mall 운영 
 * Name : TC_115
 * Scenario : 비로그인 > 마이존 선택 시 로그인 페이지 이동 확인
 * Assertion : 로그인 img alt 확인
 *
 */

public class Live_TC115 {
	...
	@Test
	public void TC115() throws Exception {
		try{
			...
			if (driver.findElement(By.xpath(".//*[@id='login_area']/form/h2/img")).getAttribute("alt").equals("로그인")) {
				System.out.println("[TC_115] success");
				assertTrue(true);
				return;
			} else {
				System.out.println("[TC_115] failure : '로그인' img alt 불일치");
				assertTrue(false);
			}
		} catch (Exception e) {
			e.printStackTrace();
			System.out.println("[TC_115] failure");
			assertTrue(false);
		}
