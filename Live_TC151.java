 ...
 * Subject : CJ Mall 운영 
 * Name : TC_151
 * Scenario : 로그인 > 마이존 > 주문내역/배송현황 전체보기 버튼 클릭시 해당 페이지 이동
 * Assertion : 주문내역/배송현황 img alt "주문내역 배송현황" 체크
 * Update : 불필요한 코드의 제거 (2016.11.11)
 *
 */

public class Live_TC151 {
	...
	@Test
	public void TC151() throws Exception {
		try {
			...
			// 주문내역/배송현황 버튼 뜰 때까지 기다림.
			wait = new WebDriverWait(driver, waitTime);
			wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath(".//*[@id='midLeft']/div/ul/li[1]/ul/li[1]/a")));
			System.out.println("주문내역/배송현황 버튼 뜰 때까지 기다림");
			
			// 주문내역/배송현황 버튼 클릭
			driver.findElement(By.xpath(".//*[@id='midLeft']/div/ul/li[1]/ul/li[1]/a")).click();
			System.out.println("주문내역/배송현황 버튼 클릭");
			
			// 주문내역/배송현황 img 뜰 때까지 기다림.
			wait = new WebDriverWait(driver, waitTime);
			wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath(".//*[@id='sub2_right']/div/div[1]/h1/img")));
			System.out.println("주문내역/배송현황 img 뜰 때까지 기다림");
						
			// 주문내역/배송현황 img alt "주문내역 배송현황" 체크
			if ("주문내역 배송현황".equals(driver.findElement(By.xpath(".//*[@id='sub2_right']/div/div[1]/h1/img")).getAttribute("alt"))) {
				System.out.println("[TC_151] success");
				assertTrue(true);
				return;
			} else {
				System.out.println("[TC_151] failure : 주문내역/배송현황 img alt 불일치");
				assertTrue(false);
			}
		} catch (Exception e) {
			e.printStackTrace();
			System.out.println("[TC_151] failure");
			assertTrue(false);
		}
	}
