 ...
 * Subject : CJ Mall 운영 
 * Name : TC_114
 * Scenario : 로그인 > 마이존 선택 시 마이존 페이지로 이동 확인 
 * Assertion : MY ZONE img alt 확인
 *
 */

public class Live_TC114 {
	...
	@Test
	public void TC114() throws Exception {
		try {
			...
			System.out.println("마이존 img 뜰 때까지 기다림");
			
			// MY ZONE img alt 확인
			if ("MY ZONE".equals(driver.findElement(By.xpath(".//*[@id='wrap']/div[1]/a/img")).getAttribute("alt"))) {
				System.out.println("[TC_114] success");
				assertTrue(true);
				return;
			} else {
				System.out.println("[TC_114] failure : 'MY ZONE' img alt 불일치");
				assertTrue(false);
			}
		} catch (Exception e) {
			e.printStackTrace();
			System.out.println("[TC_114] failure");
			assertTrue(false);
		}
	}
