 ...
 * Subject : CJ Mall 운영 
 * Name : TC_111
 * Scenario : 비로그인 > '로그인'노출, 선택 시 로그인 페이지 이동 확인
 * Assertion : 로그인 img alt 확인
 *
 */

public class Live_TC111 {
	...
	public void TC111() throws Exception {
		try{
			...
			// 로그인 페이지 뜨고, 로그인 img alt 뜰 때까지 기다림.
			wait = new WebDriverWait(driver, waitTime);
			wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath(".//*[@id='login_area']/form/h2/img")));
			System.out.println("로그인 페이지 뜨고, 로그인 img alt 뜰 때까지 기다림");
			
			// 로그인 img alt 확인
			if (driver.findElement(By.xpath(".//*[@id='login_area']/form/h2/img")).getAttribute("alt").equals("로그인")) {
				System.out.println("[TC_111] success");
				assertTrue(true);
				return;
			} else {
				System.out.println("[TC_111] failure : '로그인' Text 불일치");
				assertTrue(false);
			}
		} catch (Exception e) {
			e.printStackTrace();
			System.out.println("[TC_111] failure");
			assertTrue(false);
		}
	}
