 ...
 * Subject : CJ Mall 운영 
 * Name : TC_008
 * Scenario : "퍼스트룩" 클릭시 퍼스트룩 메인화면 전환 확인
 * Assertion : 브라우저 Title "1st Look - FIRST & EXCLUSIVE" 체크
 *
 */

public class Live_TC008 {
	...
	public void TC008() throws Exception {
		try{
			...
			// 퍼스트룩 클릭
			driver.findElement(By.xpath(".//*[@id='cjo_wrap']/div[2]/div/div/div[2]/ul/li[8]/a")).click();
			System.out.println("퍼스트룩 클릭");
		    ...
		    // 브라우저 Title "1st Look - FIRST & EXCLUSIVE" 체크
		    boolean isExist = existElement(driver, By.xpath(".//*[@id='f_logo']/a/img"), "퍼스트룩");
			if (isExist && driver.getTitle().equals("1st Look - FIRST & EXCLUSIVE")) {
				System.out.println("[TC_008] success");
				assertTrue(true);
				return;
			} else {
				System.out.println("[TC_008] failure : 브라우저 Title '1st Look - FIRST & EXCLUSIVE' 불일치");
				assertTrue(false);
			}
			
		} catch (Exception e) {
			e.printStackTrace();
			System.out.println("[TC_008] failure");
			assertTrue(false);
		}
