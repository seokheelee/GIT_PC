 ...
 * Subject : CJ Mall 운영 
 * Name : TC_013
 * Scenario : 검색창 선택 시 노출문구 소멸 확인
 * Assertion : 검색창 inputbox 클릭후 null 체크
 *
 */

public class Live_TC013 {
	...
	public void TC013() throws Exception {
		try{
			...
			// 검색창 노출문구 확인 주석
			System.out.println("검색창 노툴문구 확인 주석="+driver.findElement(By.xpath(".//*[@id='srch_disp_value']")).getAttribute("value"));
			
			// 검색창 inputbox 값 체크
			if(!"".equals(driver.findElement(By.xpath(".//*[@id='srch_disp_value']")).getAttribute("value"))){
				driver.findElement(By.xpath(".//*[@id='srch_disp_value']")).click();
				if("".equals(driver.findElement(By.xpath(".//*[@id='srch_disp_value']")).getAttribute("value"))){
					// 검색창 null 확인 주석
					System.out.println("val2="+driver.findElement(By.xpath(".//*[@id='srch_disp_value']")).getAttribute("value"));
					System.out.println("[TC_013] success");
					assertTrue(true);
					return;
				} else {
					System.out.println("[TC_013] failure : 검색창 클릭 시 null 표시되지않음");
					assertTrue(false);
				}
			} else {
				System.out.println("[TC_013] failure : 검색창 노출문구 null...");
				assertTrue(false);
			}
		} catch (Exception e) {
			e.printStackTrace();
			System.out.println("[TC_013] failure");
			assertTrue(false);
		}
	}
