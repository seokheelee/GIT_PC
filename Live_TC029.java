 ...
 * Subject : CJ Mall 운영 
 * Name : TC_029
 * Scenario : 오클락 딜 탭 클릭시 오클락 딜 메인화면 전환 확인
 * Assertion : 브라우저 Title "CJ오클락! 매일매일 오픈" 체크
 *
 */

public class Live_TC029 {
	...
	public void TC029() throws Exception {
		try{
			...
			wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath(".//*[@id='cjo_wrap']/div[2]/div/div/div[3]/ul/li[2]/a")));
			System.out.println("오클락 딜 버튼 뜰 때까지 기다림");
			...
			// 오클락딜 클릭
			driver.findElement(By.xpath(".//*[@id='cjo_wrap']/div[2]/div/div/div[3]/ul/li[2]/a")).click();
			System.out.println("오클락딜 클릭");
			
			// O'CLOCK img 뜰 때까지 기다림.
			wait = new WebDriverWait(driver, waitTime);
			wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath(".//*[@id='oclockAllWrap']/div[1]/h1/a")));
			System.out.println("O'CLOCK img 뜰 때까지 기다림");
			
			// 브라우저 Title "CJ오클락! 매일매일 오픈" 체크
			if (driver.getTitle().equals("CJ오클락! 매일매일 오픈")) {
				System.out.println("[TC_029] success");
				assertTrue(true);
				return;
			} else {
				System.out.println("[TC_029] failure : 브라우저 Title 'CJ오클락! 매일매일 오픈' 불일치");
				assertTrue(false);
			}
		} catch (Exception e) {
			e.printStackTrace();
			System.out.println("[TC_029] failure");
			assertTrue(false);
		}
	}
