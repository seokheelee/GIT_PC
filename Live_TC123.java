 ...
 * Subject : CJ Mall 운영 
 * Name : TC_123
 * Scenario : 로그인 > 마이존 > CJ ONE 포인트  노출
 * Assertion : "CJ ONE 포인트 ~P" Text 체크
 *
 */

public class Live_TC123 {
	...
	@Test
	public void TC123() throws Exception {
		try {
			...
			wait = new WebDriverWait(driver, waitTime);
			wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath(".//*[@id='wrap']/div[1]/div[2]/div[2]/div[2]/table/tbody/tr[2]/th[2]/strong/a")));
			System.out.println("CJ ONE 포인트 뜰 때까지 기다림");
			
			// "CJ ONE 포인트 ~P" Text 체크
			if ("CJ ONE 포인트".equals(driver.findElement(By.xpath(".//*[@id='wrap']/div[1]/div[2]/div[2]/div[2]/table/tbody/tr[2]/th[2]/strong/a")).getText())) {
				System.out.println("[TC_123] success");
				assertTrue(true);
				return;
			} else {
				System.out.println("[TC_123] failure : 'CJ ONE 포인트' Text 불일치");
				assertTrue(false);
			}
		} catch (Exception e) {
			e.printStackTrace();
			System.out.println("[TC_123] failure");
			assertTrue(false);
		}
	}
