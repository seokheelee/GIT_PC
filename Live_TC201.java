 ...
 * Subject : CJ Mall 운영 
 * Name : TC_201
 * Scenario : 로그인 > 마이존 > 쇼핑통장 > CJ ONE 포인트 > 당월 소멸예정인 포인트 노출
 * Assertion : 당월 소멸예정 포인트 : "~P" Text 체크
 *
 */

public class Live_TC201 {
	...
	public void TC201() throws Exception {
		try {
			...
			// CJ ONE 포인트 버튼 뜰 때까지 기다림.
			wait = new WebDriverWait(driver, waitTime);
			wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath(".//*[@id='midLeft']/div/ul/li[2]/ul/li[3]/a")));
			System.out.println("CJ ONE 포인트 버튼 뜰 때까지 기다림");
			
			// CJ ONE 포인트 클릭
			driver.findElement(By.xpath(".//*[@id='midLeft']/div/ul/li[2]/ul/li[3]/a")).click();
			System.out.println("CJ ONE 포인트 클릭");
			
			// 당월 소멸예정 포인트 뜰 때까지 기다림.
			wait = new WebDriverWait(driver, waitTime);
			wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath(".//*[@id='sub2_right']/div/div[3]/table/tbody/tr[2]/td[1]")));
			System.out.println("당월 소멸예정 포인트 뜰 때까지 기다림");
			
			// 당월 소멸예정 포인트 : "~P" Text 체크
			if ("당월 소멸예정 포인트".equals(driver.findElement(By.xpath(".//*[@id='sub2_right']/div/div[3]/table/tbody/tr[2]/td[1]")).getText())) {
				System.out.println("[TC_201] success");
				assertTrue(true);
				return;
			} else {
				System.out.println("[TC_201] failure : '당월 소멸예정 포인트 : ~P' Text 불일치");
				assertTrue(false);
			}
		} catch (Exception e) {
			e.printStackTrace();
			System.out.println("[TC_201] failure");
			assertTrue(false);
		}
	}
