 ...
 * Subject : CJ Mall 운영 
 * Name : TC_110
 * Scenario : 로그인 > '로그아웃'노출, 선택 시 로그아웃되며 메인 페이지로 이동 확인
 * Assertion : "로그아웃" Text 체크
 *
 */

public class Live_TC110 {
	...
	@Test
	public void TC110() throws Exception {
		try{
			...
			// 로그아웃 노출 확인
			if("로그아웃".equals(driver.findElement(By.xpath(".//*[@id='cjo_wrap']/div[2]/div/div/div[2]/ul/li[1]/a/span")).getText())){
				System.out.println("[TC_110] success");
				assertTrue(true);
				return;
			} else {
				System.out.println("[TC_110] failure : '로그아웃' Text 불일치");
				assertTrue(false);
			}
		} catch (Exception e) {
			e.printStackTrace();
			System.out.println("[TC_110] failure");
			assertTrue(false);
		}
	}
