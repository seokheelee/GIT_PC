 ...
 * Subject : CJ Mall 운영 
 * Name : TC_133
 * Scenario : 로그인 > 마이존 > 예치금 클릭시 해당 탭 이동 확인
 * Assertion : "예치금" img alt 확인
 *
 */

public class Live_TC133 {
	...
	public void TC133() throws Exception {
		try {
			...
			// 예치금 클릭
			driver.findElement(By.xpath(".//*[@id='wrap']/div[1]/div[2]/div[2]/div[2]/table/tbody/tr[2]/th[3]/strong/a")).click();
			System.out.println("예치금 클릭");
			
			// 예치금 뜰 때까지 기다림.
			wait = new WebDriverWait(driver, waitTime);
			wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath(".//*[@id='sub2_right']/div/div[2]/h1/a/img")));
			System.out.println("예치금 뜰 때까지 기다림");
						
			// "예치금" img alt 확인
			if ("예치금".equals(driver.findElement(By.xpath(".//*[@id='sub2_right']/div/div[2]/h1/a/img")).getAttribute("alt"))) {
				System.out.println("[TC_133] success");
				assertTrue(true);
				return;
			} else {
				System.out.println("[TC_133] failure : '예치금' img alt 불일치");
				assertTrue(false);
			}
		} catch (Exception e) {
			e.printStackTrace();
			System.out.println("[TC_133] failure");
			assertTrue(false);
		}
	}
