 ...
 * Subject : CJ Mall 운영 
 * Name : TC_006
 * Scenario : "이벤트" 클릭시 이벤트 메인화면 전환 확인
 * Assertion : "이벤트존" img alt 체크
 *
 */

public class Live_TC006 {
	...
	public void TC006() throws Exception {
		try{
			...
			// 이벤트 클릭
			driver.findElement(By.xpath(".//*[@id='cjo_wrap']/div[2]/div/div/div[2]/ul/li[6]/a/strong")).click();
			System.out.println("이벤트 클릭");
			
			// 이벤트존 페이지 뜨고, 이벤트존 객체 기다림			
			wait = new WebDriverWait(driver, waitTime);
			wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath(".//*[@id='cjm_container']/div[1]/div[2]/div/div[1]/h1/a/img")));
			System.out.println("이벤트존 페이지 뜨고, 이벤트존 객체 기다림");	
			
			// "이벤트존" img alt 체크
			boolean isExist = existElement(driver, By.xpath(".//*[@id='cjm_container']/div[1]/div[2]/div/div[1]/h1/a/img"), "이번트존");
			if (isExist && driver.findElement(By.xpath(".//*[@id='cjm_container']/div[1]/div[2]/div/div[1]/h1/a/img")).getAttribute("alt").equals("이벤트존")) {
				...
				System.out.println("[TC_006] failure : img alt 이벤트존 불일치");
				assertTrue(false);
			}
		} catch (Exception e) {
			e.printStackTrace();
			System.out.println("[TC_006] failure");
			assertTrue(false);
		}
	}
